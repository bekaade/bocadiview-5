...
/** NUEVO: de qué mercado viene el símbolo activo */
export type AssetClass = "crypto" | "cedear";
...
export interface IndicatorConfig {
  ...
  vwapProfileOffset: number;
  vwapProfileBins: number;
  vwapProfilePocType: "+VWAP" | "-VWAP" | "+/-VWAP";
}
...
export const DEFAULT_CONFIG: IndicatorConfig = {
  ...
  vwapProfileOffset: 10,
  vwapProfileBins: 50,
  vwapProfilePocType: "+/-VWAP",
};
...
interface ChartState {
  symbol: string;
  /** NUEVO: mercado del símbolo activo (crypto vía Binance, cedear vía data912) */
  ...
  watchlist: string[];
  watchlistAssetClasses: Record<string, AssetClass>;
  ...
  // Actions
  /** NUEVO: setSymbol ahora recibe también el assetClass (default "crypto" para no romper llamadas viejas) */
  setSymbol: (s: string, assetClass?: AssetClass) => void;
  setTimeframe: (t: Timeframe) => void;
  toggleIndicator: (key: IndicatorKey) => void;
  removeIndicator: (key: IndicatorKey) => void;
  toggleHidden: (key: IndicatorKey) => void;
  setConfig: (patch: Partial<IndicatorConfig>) => void;
  addToWatchlist: (s: string, assetClass?: AssetClass) => void;
...
export const useChartStore = create<ChartState>()(
  ...
  hidden: {
      ...
      watchlistAssetClasses: {},
      tool: "cursor",
      priceLines: [],
      symbolDialogOpen: false,
      settingsTarget: null,

      setSymbol: (symbol, assetClass = "crypto") => set({ symbol, assetClass }),
      ...
      addToWatchlist: (s, assetClass = "crypto") =>
        set((state) => ({
          watchlist: state.watchlist.includes(s)
            ? state.watchlist
            : [...state.watchlist, s],
          watchlistAssetClasses: {
            ...state.watchlistAssetClasses,
            [s]: assetClass,
          },
        })),
      removeFromWatchlist: (s) =>
        set((state) => {
          const { [s]: _removed, ...watchlistAssetClasses } =
            state.watchlistAssetClasses;
          return {
    ...
    }),
    {
      ...
      partialize: (s) => ({
        ...
        watchlistAssetClasses: s.watchlistAssetClasses,
      }),
    },
  ),
);
