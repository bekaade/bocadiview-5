...
export function Watchlist() {
  ...
  const watchlistAssetClasses = useChartStore((s) => s.watchlistAssetClasses);
  ...
  useEffect(() => {
    ...
    const cryptoSymbols = watchlist.filter(
      (s) => (watchlistAssetClasses[s] ?? "crypto") === "crypto",
    );
    const cedearSymbols = watchlist.filter(
      (s) => watchlistAssetClasses[s] === "cedear",
    );

    Promise.all([
      cryptoSymbols.length ? fetchTickers24h(cryptoSymbols) : Promise.resolve([]),
      cedearSymbols.length ? fetchLivePanel("arg_cedears") : Promise.resolve([]),
    ])
      .then(([tickers, cedears]) => {
        if (cancelled) return;
        ...
        cedears.forEach((item) => {
          if (cedearSymbols.includes(item.symbol)) {
            map[item.symbol] = { symbol: item.symbol, price: item.c, pct: item.pct_change };
          }
        });
        setRows(map);
      })
    ...
    const unsub = cryptoSymbols.length
      ? ws.subscribeMiniTickers(cryptoSymbols, (tick) => {
      setRows((prev) => {
  ...
  }, [watchlist, watchlistAssetClasses]);
    ...
    <div className="flex h-full flex-col">
      ...
      <ScrollArea className="flex-1">
        <div className="flex flex-col">
          {watchlist.map((s) => {
            const assetClass = watchlistAssetClasses[s] ?? "crypto";
            const quote = assetClass === "cedear" ? "ARS" : "USDT";
            const row = rows[s];
            ...
            return (
              <div
                key={`${assetClass}-${s}`}
                onClick={() => setSymbol(s, assetClass)}
                className={cn(
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-tv-text-dim">{quote}</span>
                </div>
                <span
                  className={cn(
